...
CREATE PROCEDURE INSERTAR_VETERINARIO(
    ...
    IN _rol ENUM('ADMINISTRADOR', 'VETERINARIO'),
    IN _cmpv VARCHAR(30),
...
BEGIN
    ...
    VALUES(_username, _contrasena_hash, _nombres, _apellidos, _telefono, 1, _rol, NOW(), NOW(), _modified_by);
    SET _id_usuario = LAST_INSERT_ID();
    INSERT INTO veterinario(id_veterinario, cmpv, especialidad)
    VALUES(_id_usuario, _cmpv, _especialidad);
END$$
...
BEGIN
    UPDATE usuario SET username = _username, contrasena_hash = _contrasena_hash,
        nombres = _nombres, apellidos = _apellidos, telefono = _telefono,
        modified_on = NOW(), modified_by = _modified_by
    WHERE id_usuario = _id_usuario;
    UPDATE veterinario SET cmpv = _cmpv, especialidad = _especialidad
    WHERE id_veterinario = _id_usuario;
END$$
...
-- =========================================================
-- PROCEDIMIENTOS ALMACENADOS - RECORDATORIO
-- =========================================================

CREATE PROCEDURE INSERTAR_RECORDATORIO(
    OUT _id_recordatorio INT,
    IN _fecha_programada DATETIME,
    IN _canal ENUM('WHATSAPP'),
    IN _estado_seguimiento ENUM('PENDIENTE', 'ENVIADO', 'CONTACTADO', 'CERRADO'),
...
BEGIN
    ...
    VALUES(_fecha_programada, _canal, _estado_seguimiento, _mensaje, _id_cita, NOW(), NOW(), _modified_by);
    SET _id_recordatorio = LAST_INSERT_ID();
END$$

CREATE PROCEDURE MODIFICAR_RECORDATORIO(
    IN _id_recordatorio INT,
    IN _fecha_programada DATETIME,
    IN _canal ENUM('WHATSAPP'),
    IN _estado_seguimiento ENUM('PENDIENTE', 'ENVIADO', 'CONTACTADO', 'CERRADO'),
    IN _mensaje VARCHAR(255),
...
BEGIN
    UPDATE recordatorio SET fecha_programada = _fecha_programada, canal = _canal,
        estado_seguimiento = _estado_seguimiento, mensaje = _mensaje,
        id_cita = _id_cita, modified_on = NOW(), modified_by = _modified_by
    WHERE id_recordatorio = _id_recordatorio;
END$$
...
BEGIN
    DELETE FROM recordatorio WHERE id_recordatorio = _id_recordatorio;
END$$
